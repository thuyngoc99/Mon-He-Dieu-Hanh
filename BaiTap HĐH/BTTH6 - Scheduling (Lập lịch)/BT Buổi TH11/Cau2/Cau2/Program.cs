    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (time = 0; remain != 0;)
            {
                ...
                for (i = 0; i < n; i++)
                {
                    if (arriveTime[i] <= time && priority[i] < priority[smallest] && burstTime[i] > 0)
                    {
                        smallest = i;
                    }
                }
                ...
                remain--;
                Console.WriteLine("\nP[{0}]\t\t{1}\t\t{2}\t\t{3}\t\t{4}", smallest + 1, arriveTime[smallest], priority[smallest], burstTime[smallest], time - arriveTime[smallest] - burstTime[smallest]);
                avg += time - arriveTime[smallest] - burstTime[smallest];
